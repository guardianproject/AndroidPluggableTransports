...
import static info.pluggabletransports.dispatch.DispatchConstants.PT_TRANSPORTS_SNOWFLAKE;

public class SnowflakeTransport implements Transport {
    ...
    public void register() {
        Dispatcher.get().register(PT_TRANSPORTS_SNOWFLAKE, getClass());
    }
    ...
    public void init(Context context, Properties options) {
        ...
        if (options.containsKey(OPTION_ADDRESS))
        {
            ...
            mRemoteHost = addrParts[0];
            mRemotePort = Integer.parseInt(addrParts[1]);
        }

        mRemoteCert = options.getProperty(OPTION_CERT);
    ...
    public Connection connect(final String addr) {
        ...
        try {
            return new SnowflakeConnection(addr, InetAddress.getLocalHost(), mLocalSocksPort);
        } catch (IOException e) {
    ...
    private void initObfs4SharedLibrary(Context context) {
        ...
        try {
            ...
            Goptbundle.setenv(DispatchConstants.TOR_PT_CLIENT_TRANSPORTS, DispatchConstants.PT_TRANSPORTS_SNOWFLAKE);
    ...
    public class SnowflakeConnection implements Connection {
        ...
        public SnowflakeConnection(String remoteAddress, InetAddress localSocks, int port) throws IOException {
        ...
        public String getProxyUsername ()
        {
          ...
          //  socksUser.append(OPTION_KEY).append("\\=").append(mObfs4Key).append("\\;");
            socksUser.append(OPTION_CERT).append("\\=").append(mRemoteCert).append("\\;");
    ...
    public static void setPropertiesFromBridgeString (Properties options, String bridgeLine)
    {
        ...
        options.put(SnowflakeTransport.OPTION_ADDRESS,parts[1]);
        options.put(SnowflakeTransport.OPTION_KEY,parts[2]);
        options.put(SnowflakeTransport.OPTION_CERT,parts[3].split("=")[1]);
        options.put(SnowflakeTransport.OPTION_IAT_MODE,parts[4].split("=")[1]);
